...
class TraineeProfileScreen extends StatelessWidget {
  ...
  Widget _buildSessionItem(BuildContext context, TrainingSession session) {
    ...
    return Opacity(
      opacity: isPending ? 0.5 : 1.0, // Optionally reduce opacity for pending sessions
      child: Card(
        color: isPending ? Colors.grey.shade800 : Colors.white, // Grey out pending sessions
        child: Stack(
          children: [
            ListTile(
              leading: Icon(Icons.fitness_center, color: isPending ? Colors.grey : Colors.black),
              title: StreamBuilder<Profile>(
              ...
              trailing: Icon(Icons.arrow_forward_ios, color: isPending ? Colors.grey : Colors.black),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () {
                    Navigator.of(context).pop(); // This closes the dialog immediately.
                    _deleteTrainingSession(context, session); // Then, call your delete function.
                  },
  ...
  Future<void> _deleteTrainingSession(BuildContext context, TrainingSession session) async {
    ...
    if (confirmDelete ?? false) { // The ?? false handles nulls, ensuring a boolean value.
      try {
        await session.deleteTrainingSession(); // Assuming this is an async operation
